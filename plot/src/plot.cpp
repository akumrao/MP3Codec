...
Plot::Plot(unsigned mode)
    : filenamePrefix("plotp"),
      labelX(),
      labelY(),
      labelTitle(),
      legendVec(0),
...
void Plot::legend(std::initializer_list<string> legendVec)
{
    this->legendVec.resize(legendVec.size());
    std::copy(legendVec.begin(), legendVec.end(), this->legendVec.begin());
}

void Plot::linespec(unsigned lineIndex, LineSpecInput lineSpec)
{
...
void Plot::linespec(unsigned lineIndex, LineProperty property, string value)
{
    linespec(lineIndex, {{property, value}});
}

void Plot::linespec(unsigned lineIndex, LineProperty property, double value)
{
    linespec(lineIndex, {{property, to_string(value)}});
}

void Plot::grid(bool flag)
{
    this->isGridded = flag;
}

void Plot::plot(initializer_list<DataVector> il)
{
...
void Plot::print(const string &filenameExport)
{
    this->filenameExport = filenameExport;
}

string Plot::exportFileName()
{
    return this->filenameExport + std::to_string(instance);
}

void Plot::exec(bool run_gnuplot)
{
...
bool Plot::existsTerminal(string terminalName)
{
    bool result = false;
...
void Plot::gpScreen(bool run_gnuplot)
{
...
void Plot::gpPng(bool run_gnuplot)
{
    //* Generate gnuplot batch file
...
void Plot::gpEps(bool run_gnuplot)
{
    //* Generate gnuplot batch file
...
void Plot::gpPdf(bool run_gnuplot)
{
    //* Generate gnuplot batch file
...
void Plot::gpHtml(bool run_gnuplot)
{
    //* Generate gnuplot batch file
...
void Plot::gpSvg(bool run_gnuplot)
{
    //* Generate gnuplot batch file
...
void Plot::gpHeader(ofstream &fout)
{
...
void Plot::gpCurve(ofstream &fout, const string &filename, bool run_gnuplot)
{
